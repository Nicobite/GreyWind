...
UDPServer::UDPServer(const std::string& addr, int port): m_port(port), m_addr(addr){
    ...
    m_socket = socket(m_addrinfo->ai_family, SOCK_DGRAM | SOCK_CLOEXEC | SO_REUSEADDR, IPPROTO_UDP);
    if(m_socket == INVALID_SOCKET){
        freeaddrinfo(m_addrinfo);
        throw std::runtime_error(("Could not create socket for  \"" + addr + ":" + decimal_port + "\"").c_str());
    }
...
std::string  UDPServer::get_client_addr() const{
    return std::string(inet_ntoa(m_client.sin_addr));
}

int UDPServer::get_client_port() const{
    return ntohs(m_client.sin_port);
}

bool UDPServer::is_client_known() const{
    return m_client_is_known;
}

void UDPServer::set_client(const std::string& addr, int port){
    char decimal_port[16];
    ...
    memcpy(&m_client, client_addrinfo->ai_addr, sizeof(m_client));
    m_fromsize = client_addrinfo->ai_addrlen;
...
int UDPServer::send(const char* msg, size_t size, SOCKADDR_IN client, int fromsize){
    int n = sendto(m_socket, msg, size, 0, (SOCKADDR *) &client, fromsize);
    if(n < 0){
        throw std::runtime_error("Could not send from UDP Server socket.");
    }
    return n;
